...
import { FXRP_LABEL } from "@/lib/redemption";
...
export type RedemptionFormViewProps = {
  ...
  // Computed amount (formatted FXRP), or null when no valid lot count
...
export function RedemptionFormView(
  ...
  const redeemDisabled =
    blockedReason !== null || approvalRequired || busy;
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {/* Lot count + computed amount */}
      <div className="flex flex-col gap-2">
        <label htmlFor="lot-count" className={fieldLabelClass()}>
          Lots to redeem
        </label>
        <input
          id="lot-count"
      ...
      {submittedRequestIds !== null ? (
        <Callout tone="success" title="Redemption request submitted">
          <p>
            {submittedRequestIds.length === 1
              ? "Request id "
              : "Request ids "}
            <span className="font-mono">
              {submittedRequestIds.join(", ")}
            </span>
        ...
        <Callout tone="info" title={approvalPending ? "Approving FXRP" : "Submitting redemption"}>
      ...
      <div className="flex flex-col gap-2">
        ...
        {blockedReason !== null ? (
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">
            Approve the AssetManager to spend the exact lot amount, then redeem.
          </p>
        ) : null}
